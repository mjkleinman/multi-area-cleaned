...
class Trial(object):
    ...
    def psychometric(self, savepath=None, filename=None):
        ...
        f   = plt.figure(figsize=(3.165, 2.995))
        ax  = f.gca()

        ax.plot(2*(u_conds / 225) - 1, choose_red_rates,'k', marker='.', markersize=10)
    ...
    def reaction_time(self, savepath=None, filename=None):
        ...
        f   = plt.figure()
        ax  = f.gca()
        ax.plot(2*(u_conds / 225) - 1, rts_cond, 'k', marker='.', markersize=20)
        ax.set_xlim((-1.0, 1.0))
        ax.set_xlabel('Checkerboard coherence')
...
## JCK for getting rid of low fr.
    ...
    def tdr(self, idx1=np.hstack((np.arange(80), np.arange(240, 260))), idx2=np.hstack((np.arange(80, 160), np.arange(260, 280))), idx3=np.hstack((np.arange(160, 240), np.arange(280, 300)))):
        ...
        # for i in range(ntrials):
        #     F[0, i] = self.trials[i]['info']['choice']
        #     F[1, i] = coh_r(self.trials[i]['info']['cond']) # maybe divide by 0.9022 so that it's normalized
        #     F[2, i] = self.trials[i]['info']['left_right']
        # #     F[3, i] = psth.trials[i]['info']['left_right'] * coh(psth.trials[i]['info']['cond'])
...
#       self.WrEvals = WrEvals
    ...
    def plot_local_dynamics_delta(self, inputs=np.array((0,0,0,0)), f=None, scale=1, force_alignment=None, t_sample=None, coh=None, reset_align=True, filters=None):
        ...
        for i in np.arange(len(x_deltas)):
            for j in np.arange(len(y_deltas)):
                ax.arrow(ss[0, i, j], ss[1, i, j], sdots[0, i, j], sdots[1, i, j], color=np.array([0.5, 0.5, 0.5]), head_width=0.05, head_length=0.1)
    ...
    def plot_neural_traj2d_partition_pub(self, window=None, f=None, filters=None, cond_color=None, cond_ls=None, cond_lw=None, savepath=None, filename=None, display=True, alphaval=1, windowStart=0, windowEnd=0, targ_on_label=False, go_cue_label=False, pc_label=False, ax_loc=None, xlim=None, ylim=None, projector='PCs', close=True, betas=None):
        ...
        for pidx, partition in enumerate(self.partitions):
            ...
            for j in idxs:
                ...
                # targ on
                if targ_on_label:
                    # if j==0: #only for one condition
                    ax.plot(pc_scores[0, windowStart+targ_on], pc_scores[1, windowStart+targ_on], color=(0.4,0.7,1), marker='.', markersize=20)
                # go-cue
                if go_cue_label:
                    # if j==0 or j==13:
                    ax.plot(pc_scores[0, windowStart+go], pc_scores[1, windowStart+go], color=(0.4,0.7,0.4), marker='.', markersize=20)
