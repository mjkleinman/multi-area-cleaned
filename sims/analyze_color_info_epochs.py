...
if cfg_mk['num_seeds'] > -1:
    all_files = [cfg_mk['modelpath'] + cfg_mk['suffix'] + '_seed=' + str(seed) + '_epoch=' + str(epoch) + '.pkl' for epoch in range(num_epochs)]
else:
...
for mm in range(len(all_files)):
    ...
    call('rm -f *_copy.pkl')
    bounds = [200, 700]  # used to be -500, 500

    time_mv = psth.cbs + [int(trial['info']['epochs']['check'][0]) for trial in psth.trials]
...
data_save_path = "/Users/michael/Documents/GitHub/multi-area/sims/revision/scratch_data_mi"
...
# pdb.set_trace()
np.save(cfg_mk['modelpath'] + cfg_mk['suffix'] + '_color_epoch_seed' + str(seed) + suffix_linear + '.npy', 1 - (mi_store_color * 1.44))
np.save(cfg_mk['modelpath'] + cfg_mk['suffix'] + '_direction_epoch_seed' + str(seed) + suffix_linear + '.npy', 1 - (mi_store * 1.44))
np.save(cfg_mk['modelpath'] + cfg_mk['suffix'] + '_acc_color_epoch_seed' + str(seed) + suffix_linear + '.npy', acc_store_color)
np.save(cfg_mk['modelpath'] + cfg_mk['suffix'] + '_acc_direction_epoch_seed' + str(seed) + suffix_linear + '.npy', acc_store)
