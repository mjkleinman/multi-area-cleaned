...
overlap12_store = np.zeros((8, 3, 120))
...
for seed in range(8):
    ...
    dpca_axes1 = np.array([dpca1.P['tc'], dpca1.P['td']]).reshape(2, 120).T
    # dpca_axes2 = np.array([dpca2.P['tc'], dpca2.P['td']]).reshape(2, 80).T
    # dpca_axes3 = np.array([dpca3.P['tc'], dpca3.P['td']]).reshape(2, 80).T

    overlap12_store[seed, :, :] = getOverlap(dpca_axes1, W12, num_exc=120)
...
np.save(savepath + 'null_potent_dpca_twoarea.npy', overlap12_store)
