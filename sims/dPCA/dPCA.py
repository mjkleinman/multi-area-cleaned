...
class dPCA(BaseEstimator):
    ...
    # for dpca which axes do I orthogonalize
    # TODO: there's a bug in this implementation, as it depends on the sign of P for P*D*X
    # June 5, 2020
    def calculate_variance_with_time(self, X):
        Xr = X.reshape(X.shape[0], -1)
        Xzm = - np.mean(Xr, axis=1).reshape(-1, 1) + Xr
        var_tot = np.sum(Xzm ** 2)
        col_var = None
        ...
        c = np.array([self.P['c'].reshape(-1), self.P['tc'].reshape(-1)])
        d = np.array([self.P['d'].reshape(-1), self.P['td'].reshape(-1)])
        x = np.array([self.P['dc'].reshape(-1), self.P['tdc'].reshape(-1)])

        qc, _ = np.linalg.qr(c.T)
        qd, _ = np.linalg.qr(d.T)
        qx, _ = np.linalg.qr(x.T)

        frac_var_c = dPCA.getVariance_helper(qc[:,0].reshape(-1,1), self.D['c'], Xzm, var_tot)
        frac_var_tc = dPCA.getVariance_helper(qc[:,1].reshape(-1,1), self.D['tc'], Xzm, var_tot)
        frac_var_d = dPCA.getVariance_helper(qd[:,0].reshape(-1,1), self.D['c'], Xzm, var_tot)
        frac_var_td = dPCA.getVariance_helper(qd[:,1].reshape(-1,1), self.D['tc'], Xzm, var_tot)
        frac_var_x = dPCA.getVariance_helper(qx[:,0].reshape(-1,1), self.D['tdc'], Xzm, var_tot)
        frac_var_tx = dPCA.getVariance_helper(qx[:,1].reshape(-1,1), self.D['tdc'], Xzm, var_tot)

        return frac_var_c + frac_var_tc, frac_var_d + frac_var_td, frac_var_x + frac_var_tx
