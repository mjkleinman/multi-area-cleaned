...
mi_store_context = np.zeros((len(all_files), ))

acc_store = np.zeros((len(all_files), ))
acc_store_color = np.zeros((len(all_files), ))
acc_store_context = np.zeros((len(all_files), ))
...
for mm in range(len(all_files)):
    ...
    # color information
    num_s1_x = len(idx1x[0])
    num_s2_x = len(idx2x[0])
    y = np.zeros((num_s1_c + num_s2_c))
    y[:num_s1_x] = 1
    y[num_s1_x:] = 0
    ...
    x = np.concatenate((s1, s2), axis=0)
    X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.75, random_state=42)
    mi_store_color[mm], acc_store_context[mm] = nnDecode(X_train, y_train, X_test, y_test, cfg_mk['linear_mut_info'])
...
np.save(cfg_mk['modelpath'] + cfg_mk['suffix'] + '_context' + suffix_linear + '.npy', 1 - (mi_store_context * 1.44))
...
np.save(cfg_mk['modelpath'] + cfg_mk['suffix'] + '_acc_context' + suffix_linear + '.npy', acc_store_context)
