...
overlap12_store = np.zeros((8, 3, 100))
# overlap21_store = np.zeros((5, 8, 3, 80))
overlap23_store = np.zeros((8, 3, 100))
overlap11_store = np.zeros((8, 3, 100))
overlap22_store = np.zeros((8, 3, 100))
overlap33_store = np.zeros((8, 3, 100))

# overlap32_store = np.zeros((5, 8, 3, 80))


def getOverlap(dpca_axes, matrix, num_exc=80):
    # computes overlap between axes and feedforward connectivity matrix
    overlap_store = np.zeros((3, num_exc))
    for rank in range(num_exc):
...
for seed in range(8):
    ...
    Wout = psth.rnn.Wout
    Wrec = psth.rnn.Wrec
    W12 = Wrec[100:200, 0:100]  # To, From
    W11 = Wrec[0:100, 0:100]
    W22 = Wrec[100:200, 100:200]
    W33 = Wrec[200:300, 200:300]
    # W21 = Wrec[0:80, 80:160]
    W23 = Wrec[200:300, 100:200]
    # W32 = Wrec[80:160, 160:240]

    idx1 = np.hstack((np.arange(100)))
    idx2 = np.hstack((np.arange(100, 200)))
    idx3 = np.hstack((np.arange(200, 300)))
    ...
    dpca_axes1 = np.array([dpca1.P['tc'], dpca1.P['td']]).reshape(2, 100).T
    dpca_axes2 = np.array([dpca2.P['tc'], dpca2.P['td']]).reshape(2, 100).T
    dpca_axes3 = np.array([dpca3.P['tc'], dpca3.P['td']]).reshape(2, 100).T

    overlap12_store[seed, :, :] = getOverlap(dpca_axes1, W12, num_exc=100)
    overlap23_store[seed, :, :] = getOverlap(dpca_axes2, W23, num_exc=100)
    overlap11_store[seed, :, :] = getOverlap(dpca_axes1, W11, num_exc=100)
    overlap22_store[seed, :, :] = getOverlap(dpca_axes2, W22, num_exc=100)
    overlap33_store[seed, :, :] = getOverlap(dpca_axes3, W33, num_exc=100)

    # overlap21_store[p, seed, :, :] = getOverlap(dpca_axes2, W21)
    # overlap32_store[p, seed, :, :] = getOverlap(dpca_axes3, W32)


savepath = cfg_mk['path'] + 'sims/neurips/'
np.save(savepath + 'null_potent_dpca_nodale.npy', overlap12_store)
np.save(savepath + 'null_potent_dpca_nodale_l2.npy', overlap23_store)
np.save(savepath + 'null_potent_dpca_nodale_l11.npy', overlap11_store)
np.save(savepath + 'null_potent_dpca_nodale_l22.npy', overlap22_store)
np.save(savepath + 'null_potent_dpca_nodale_l33.npy', overlap33_store)
