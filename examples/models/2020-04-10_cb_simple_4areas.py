...
INH_SENSORY = INH[:Ninh // 4]
EXC_PREMOTOR = EXC[Nexc // 4:2 * Nexc // 4]
INH_PREMOTOR = INH[Ninh // 4:2 * Ninh // 4]
EXC_MOTOR = EXC[2 * Nexc // 4:3 * Nexc // 4]
INH_MOTOR = INH[2 * Ninh // 4:3 * Ninh // 4]
EXC_SPINE = EXC[3 * Nexc // 4:]
INH_SPINE = INH[3 * Ninh // 4:]
...
for i in EXC_MOTOR:
    Crec[i, EXC_MOTOR] = 1
    Crec[i, i] = 0
    Crec[i, EXC_SPINE] = 1 * (rng.uniform(size=len(EXC_SPINE)) < fb_prop)  # these are back projections
    Crec[EXC_SPINE, i] = 1 * (rng.uniform(size=len(EXC_SPINE)) < ff_prop)  # these are forward projections
    Crec[i, INH_MOTOR] = np.sum(Crec[i, EXC_MOTOR]) / len(INH_MOTOR)
for i in EXC_SPINE:
    Crec[i, EXC_SPINE] = 1
    Crec[i, i] = 0
    Crec[i, INH_SPINE] = np.sum(Crec[i, EXC_SPINE]) / len(INH_SPINE)
...
for i in INH_SPINE:
    Crec[i, EXC_SPINE] = 1
    Crec[i, INH_SPINE] = np.sum(Crec[i, EXC_SPINE]) / (len(INH_SPINE) - 1)
    Crec[i, i] = 0
