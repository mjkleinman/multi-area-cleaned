# This fixes the checkerboard and other things like start time so that 
# plots look pretty. 
...
cb_drawn_bounds = [600, 600]#[600, 1000]  
...
start_delay_var = 0 #1 # in ms, variance of start time.
...
#### Question to investigate: does this enforce Dale's law?
...
for i in EXC_SENSORY:
    ...
    Crec[i,i]           = 0
#    Crec[i,EXC_PREMOTOR]   = 1*(rng.uniform(size=len(EXC_PREMOTOR)) < 0.2) #these are backprojections
    Crec[EXC_PREMOTOR, i] = 1 # fully connected projections for now.
    Crec[i,INH_SENSORY] = np.sum(Crec[i,EXC_SENSORY])/len(INH_SENSORY)
for i in EXC_PREMOTOR:
    Crec[i,EXC_PREMOTOR] = 1
    Crec[i,i]         = 0
#    Crec[i,EXC_MOTOR]   = 1*(rng.uniform(size=len(EXC_MOTOR)) < 0.2) #these are backprojections
    Crec[EXC_MOTOR, i] = 1
