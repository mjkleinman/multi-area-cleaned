...
ff_prop = 0.01  # feed forward proportion connectivity
fb_prop = 0.005  # feed back connectivity
...
for i in SENSORY:
    Crec[i, SENSORY] = 1
    Crec[i, i] = 0
    Crec[i, PREMOTOR] = 1 * (rng.uniform(size=len(PREMOTOR)) < fb_prop)  # these are back projections
    Crec[PREMOTOR, i] = 1 * (rng.uniform(size=len(PREMOTOR)) < ff_prop)  # these are forward projections.
    # Crec[i, INH_SENSORY] = np.sum(Crec[i, SENSORY]) / len(SENSORY)
for i in PREMOTOR:
    Crec[i, PREMOTOR] = 1
    Crec[i, i] = 0
    Crec[i, MOTOR] = 1 * (rng.uniform(size=len(MOTOR)) < fb_prop)  # these are back projections
    Crec[MOTOR, i] = 1 * (rng.uniform(size=len(MOTOR)) < ff_prop)  # these are forward projections
